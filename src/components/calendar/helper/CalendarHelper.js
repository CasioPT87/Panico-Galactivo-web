...
export default class CalendarHelper {
  ...
  numberOfRowsForMonthTable() {
    ...
    const totalNumberOfDays =
      numberOfWeekFirstDayMonth + this.dateTable.daysInMonth();
  ...
  dayObjectsForMonthTable() {
    ...
    const [daysPreviousMonth, daysNextMonth] = this.getDisabledDaysForTable(
      dayOfWeekFirstDayOfMonth,
      dayOfWeekLastDayOfMonth
    );
    const indexedDays = this.setIndexDayObjectsForTable([
      ...daysPreviousMonth,
      ...daysMonth,
      ...daysNextMonth,
    ]);
  ...
  getDisabledDaysForTable(numDaysPreviousMonth, numDaysNextMonth) {
    ...
    if (numDaysPreviousMonth > 0) {
      ...
      daysPreviousMonth = this.dayObjectsForMonth(
        previousMonth,
    ...
    if (numDaysNextMonth < weekLength - 1) {
      ...
      daysNextMonth = this.dayObjectsForMonth(
        nextMonth,
        null,
        weekLength - 1 - numDaysNextMonth,
        true
      );
    }

...
export default function ({ children, helper, numRows, tableDays, weekLength }) {
  ...
  const daysOrderedByRow = () => {
    ...
    for (let i = 0; i < numRows; i++) {
      daysOrderedByRow[i] = tableDays.slice(
        i * weekLength,
        (i + 1) * weekLength
      );
    }
  ...
  const rowDayNames = () => {
    return Object.values(DAYS_WEEK).map((day) => {
      return (
        <div key={day[1]} className={styles.c_boxes__day_name}>
          {day[1]}
        </div>
      );
    });
  ...
  const rowMonthName = () => {
    return (
      <div
        className={styles.c_boxes__month_name}
      >{`${helper.getMonthDisplay()}, ${helper.getYearDisplay()}`}</div>
    );
  };

  return (
    <div style={{ width: size }} className={styles.c_boxes}>
      <div className={styles.c_boxes__row}>{rowMonthName()}</div>
      <div className={styles.c_boxes__row}>{rowDayNames()}</div>
